...
def identity_processor(request, num_cores, tmpdir):
    ...
    if IS_ALIGNED_PARAM_NAME in request.fixturenames and not request.getfixturevalue(
        IS_ALIGNED_PARAM_NAME
    ):
    ...
    return IdentityProcessor(path_reads_file, cores=num_cores, outfile=path_output_file)
